...
import { Flex, Text, Image, useColorMode, useColorModeValue } from '@chakra-ui/react';
...
function VideoItem({ data }) {
    ...
    useEffect(() => {
        ...
        if (userId) getUserInfo(firebaseFireStore, userId).then((data) => {
            setUserInfo(data);
        });
    ...
    return (
        <Flex
            ...
            _hover={{ shadow: 'md' }}
        >
            <Link to={`/dashboard/videoDetail/${data.id}`} style={{ backgroundColor: '#000', height: '160px', width: '100%', display: 'flex', alignItems: 'center' }}
            >
            ...
            >
                ...
                <Flex
                    ...
                    alignItems={'center'}
                >
                    <Text color={textColor} isTruncated fontSize={20}>{data.title}</Text>
